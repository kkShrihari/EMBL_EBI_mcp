...
// src/handlers/search/handlers/search_all.ts
function removeZeroHits(node) {
  if (node.hitCount === 0) return null;
  if (Array.isArray(node.subdomains)) {
    node.subdomains = node.subdomains.map(removeZeroHits).filter(Boolean);
  }
  return node;
}
function pruneTree(node, depth) {
  ...
  return {
    ...node,
    subdomains: node.subdomains.map(
      (s2) => pruneTree(s2, depth - 1)
    )
  };
}
function flattenTree(node, out = []) {
  ...
  if (Array.isArray(node.subdomains)) {
    for (const s2 of node.subdomains) {
      flattenTree(s2, out);
    }
  }
  return out;
}
var EBISearchAllHandler = class {
  async run(args) {
    const {
      query,
      size = 10,
    ...
    let processedSubdomains = void 0;
    if (includeHierarchy && rootDomain?.subdomains) {
      processedSubdomains = rootDomain.subdomains.map(
        (s2) => pruneTree(s2, maxDepth)
      );
      processedSubdomains = processedSubdomains.map(removeZeroHits).filter(Boolean);
      processedSubdomains = processedSubdomains.flatMap(
        (s2) => flattenTree(s2)
      );
    }
    const payload = {
      ...
      domains: [
        {
          ...
          ...processedSubdomains ? { subdomains: processedSubdomains } : {}
        }
      ]
    };
    return {
...
// src/handlers/search/handlers/search_domain.ts
var EBISearchDomainHandler = class {
  async run(args) {
    ...
    const payload = {
      ...
      entries: data.entries.map((e2) => ({
        id: String(e2.id),
        acc: e2.acc,
        source: e2.source ?? domain,
        ...e2.fields && Object.keys(e2.fields).length > 0 ? { fields: e2.fields } : {}
      }))
    };
    return {
      content: [
...
// src/handlers/search/handlers/get_entry.ts
var EBIGetEntryHandler = class {
  async run(args) {
...
// src/handlers/search/handlers/xref_domain.ts
var EBICrossReferenceDomainHandler = class {
  async run(args) {
    const { domain } = args;
    if (!domain) {
    ...
    const payload = {
      sourceDomain: domain,
      targetDomains: data.domains.map((d) => String(d.id))
    };
    return {
      content: [
        {
...
// src/handlers/search/handlers/xref_targeted.ts
var xrefDomainHandler = new EBICrossReferenceDomainHandler();
var allowedTargetsCache = /* @__PURE__ */ new Map();
var EBICrossReferenceTargetedHandler = class {
  async run(args) {
    ...
    if (domain === "uniprot" && typeof entryIds === "string" && entryIds.includes("_")) {
      throw new Error(
        "For UniProt XREF, use accession (e.g. P04637), not entry name (P53_HUMAN)"
      );
    }
    if (!allowedTargetsCache.has(domain)) {
      const res2 = await xrefDomainHandler.run({ domain });
      if (!res2?.structuredContent?.targetDomains || !Array.isArray(res2.structuredContent.targetDomains)) {
        throw new Error(
          `Failed to discover valid xref targets for domain '${domain}'`
        );
      }
      const targets = res2.structuredContent.targetDomains.map(
        (t2) => t2.domain
      );
    ...
    const payload = {
      ...
      targetDomain,
      count: data.entries.length,
      crossReferences: data.entries.map((e2) => ({
        sourceId: String(e2.id),
        targets: (e2.references ?? []).map((x2) => ({
          id: String(x2.id),
          domain: String(x2.source)
        }))
      }))
    };
    return {
...
// src/handlers/search/handlers/xref_all.ts
var MAX_FETCH_DOMAINS = 10;
var MAX_RETURN_DOMAINS = 3;
var MAX_ENTRIES_PER_DOMAIN = 3;
var PRIORITY_DOMAINS = [
  "alphafold",
  "pdbe",
  "go"
];
var EBICrossReferenceAllHandler = class {
  async run(args) {
    ...
    const discoveredTargets = discovery?.structuredContent?.targetDomains?.map(
      (t2) => t2.domain
    ) ?? [];
    ...
    const prioritizedTargets = [
      ...
      ...discoveredTargets.filter(
        (d) => typeof d === "string" && PRIORITY_DOMAINS.indexOf(d) === -1
      )
    ];
    ...
    for (const target of targetsToFetch) {
      ...
      try {
        ...
        for (const e2 of data.entries) {
          for (const r2 of e2.references ?? []) {
            if (!grouped.has(target)) {
              grouped.set(target, /* @__PURE__ */ new Set());
            }
            grouped.get(target).add(String(r2.id));
          }
        }
      } catch {
      }
    ...
    const allCrossRefs = Array.from(grouped.entries()).map(
      ([targetDomain, ids]) => ({
        targetDomain,
        entries: Array.from(ids).slice(0, MAX_ENTRIES_PER_DOMAIN).map((id) => ({ id }))
      })
    );
...
// src/handlers/search/handlers/more_like_this_same_domain.ts
var DEFAULT_MIN = 3;
var MAX_RESULTS = DEFAULT_MIN + 3;
var EBIMoreLikeThisSameDomainHandler = class {
  async run(args) {
    const { domain, entryId } = args;
    if (!domain || !entryId) {
      throw new Error("domain and entryId are required");
    ...
    const limitedEntries = entries.slice(0, MAX_RESULTS).map((e2) => ({
      id: String(e2.id)
    }));
    const payload = {
      domain,
...
// src/handlers/search/handlers/more_like_this_cross_domain.ts
var DEFAULT_LIMIT = 3;
var MAX_LIMIT = 6;
var EBIMoreLikeThisCrossDomainHandler = class {
  async run(args) {
    const {
      domain,
      entryId,
    ...
    const relatedEntries = entries.slice(0, limit).map((e2) => ({
      id: String(e2.id)
    }));
    const payload = {
      sourceDomain: domain,
...
// src/handlers/search/handlers/summary_suggestions.ts
var DEFAULT_SIZE = 10;
var MAX_SIZE = 15;
var EBISummarySuggestionsHandler = class {
  async run(args) {
    ...
    const url = "https://www.ebi.ac.uk/ebisearch/summary/api/suggestion?" + new URLSearchParams({
      term,
      size: String(limit)
    }).toString();
    const res = await fetch(url);
...
// src/handlers/search/handlers/summary_identification.ts
var DEFAULT_SIZE2 = 5;
var MAX_SIZE2 = 15;
var EBISummaryIdentificationHandler = class {
  async run(args) {
    const { term, spine, tid } = args;
    let size = args.size ?? DEFAULT_SIZE2;
    size = Math.min(size, MAX_SIZE2);
    if (!term) {
      throw new Error("term is required");
    }
    ...
    const url = "https://www.ebi.ac.uk/ebisearch/summary/api/identification?" + params.toString();
    const res = await fetch(url);
    if (!res.ok) {
      throw new Error(
    ...
    if (data?.termIdentificationMap) {
      for (const [spineKey, entries] of Object.entries(
        data.termIdentificationMap
      )) {
        for (const e2 of entries) {
          if (Array.isArray(e2.references)) {
            for (const r2 of e2.references) {
              identifications.push({
                spine: spineKey,
                identifier: r2?.databaseEntryReference?.identifier,
                database: r2?.databaseEntryReference?.databaseName,
                label: r2?.databaseEntryReference?.label,
                name: r2?.databaseEntryReference?.name,
                species: r2?.species,
...
// src/handlers/search/handlers/summary_multireference.ts
var EBISummaryMultiReferenceHandler = class {
  async run(args) {
    ...
    const url = `https://www.ebi.ac.uk/ebisearch/ws/rest/summary/api/multireference?` + params.toString();
    const res = await fetch(url);
    if (!res.ok) {
      throw new Error(
    ...
    if (!contentType.includes("application/json")) {
      const text = await res.text();
      throw new Error(
        "EBI Summary API returned non-JSON response: " + text.slice(0, 200)
      );
    }
    const data = await res.json();
...
// src/handlers/search/handlers/summary_details.ts
var EBISummaryDetailsHandler = class {
  async run(args) {
    ...
    const url = `https://www.ebi.ac.uk/ebisearch/ws/rest/summary/api/details?` + params.toString();
    const res = await fetch(url);
    if (!res.ok) {
      throw new Error(
        `EBI summary details failed (${res.status})`
      );
    }
    const contentType = res.headers.get("content-type") ?? "";
    if (!contentType.includes("application/json")) {
    ...
    if (!Array.isArray(data.details)) {
      throw new Error(
        "Unexpected API response format: details not found"
      );
    }
    const payload = {
...
// src/handlers/Protein/handlers/proteins_.ts
var BASE_URL = "https://rest.uniprot.org/uniprotkb";
function summarizeGO(crossRefs = []) {
  return crossRefs.filter((ref) => ref.database === "GO").map((ref) => {
...
function summarizeProtein(entry) {
  return {
...
function extractIsoforms(entry, limit = 5) {
  const alt = entry.comments?.find(
    (c) => c.commentType === "ALTERNATIVE PRODUCTS"
  );
  const isoforms = alt?.isoforms ?? [];
  return isoforms.slice(0, limit).map((iso) => ({
...
var ProteinIdentityHandler = class {
  async run(args) {
    ...
    if (action === "isoforms") {
      ...
      summarizedResults = rawResults.map(summarizeProtein).slice(0, 10);
    }
    return {
      content: [
        {
...
// src/handlers/Protein/handlers/features.ts
var BASE_URL2 = "https://www.ebi.ac.uk/proteins/api";
...
function flattenFeatures(data) {
  ...
  if (Array.isArray(data)) {
    return data.flatMap(
      (entry) => Array.isArray(entry.features) ? entry.features : []
    );
  }
  return [];
}
var ProteinFeaturesHandler = class {
  async run(args) {
    ...
    switch (action) {
      case "search": {
        ...
        url = `${BASE_URL2}/features?${params.toString()}`;
        break;
      }
      case "by_type": {
        if (!type) {
        ...
        if (!query) {
          ...
          if (organism) params2.set("organism", organism);
          url = `${BASE_URL2}/features?${params2.toString()}`;
          break;
        }
        const params = new URLSearchParams({
          terms: query,
        ...
        if (organism) params.set("organism", organism);
        url = `${BASE_URL2}/features/type/${encodeURIComponent(
          type
        )}?${params.toString()}`;
        break;
      }
      case "by_accession": {
        ...
        url = `${BASE_URL2}/features/${encodeURIComponent(
          accession
        )}?${params.toString()}`;
        break;
      }
    ...
    const normalized = features.map((f3) => ({
      type: f3.type,
      category: f3.category,
      description: f3.description ?? null,
      begin: f3.begin ?? null,
      end: f3.end ?? null
    }));
    const payload = {
...
// src/handlers/Protein/handlers/proteomics.ts
var BASE_URL3 = "https://www.ebi.ac.uk/proteins/api";
...
function trimProteomicsFeatures(features, featureLimit, evidenceLimit) {
  return (features ?? []).slice(0, featureLimit).map((f3) => ({
    type: f3.type,
    begin: f3.begin,
    end: f3.end,
    peptide: f3.peptide,
    unique: f3.unique,
    evidences: (f3.evidences ?? []).slice(0, evidenceLimit).map((e2) => ({
      code: e2.code,
      source: {
        name: e2.source?.name,
        id: e2.source?.id,
        url: e2.source?.url,
        properties: e2.source?.properties ? Object.fromEntries(
          Object.entries(e2.source.properties).slice(0, 10)
        ) : void 0
      }
    }))
  }));
}
var ProteinProteomicsHandler = class {
  async run(args) {
    const size = normalizeSize2(args.size);
    let data;
    if (args.action === "species") {
      data = await fetchJson(`${BASE_URL3}/proteomics/species`);
    } else if (args.action === "species_search") {
      if (!args.query) throw new Error("query required");
      const all = await fetchJson(`${BASE_URL3}/proteomics/species`);
      const [key, value] = args.query.split(":");
      data = all.filter(
        (s2) => key === "taxId" ? String(s2.taxId) === value : key === "upId" ? s2.upId === value : false
      );
      ...
      const raw = await fetchJson(
        `${BASE_URL3}/proteomics/nonPtm/${encodeURIComponent(
          args.accession
        )}`
      );
      data = {
      ...
      if (!args.accession) throw new Error("accession required");
      const raw = await fetchJson(
        `${BASE_URL3}/proteomics/ptm/${encodeURIComponent(
          args.accession
        )}`
      );
      data = {
      ...
      if (!args.accession) throw new Error("accession required");
      const raw = await fetchJson(
        `${BASE_URL3}/proteomics/hpp/${encodeURIComponent(
          args.accession
        )}`
      );
      data = {
        ...
        features: trimProteomicsFeatures(
          raw.features,
          size,
          // ✅ default 3, max 10 features
          10
          // ✅ max 10 evidences per feature
        )
      };
    } else {
    ...
    return {
      structuredContent: {
        ...
        count: Array.isArray(data?.features) ? data.features.length : Array.isArray(data) ? data.length : 1,
        data
      },
      content: [
        {
...
// src/handlers/Protein/handlers/proteomes.ts
var BASE_URL4 = "https://rest.uniprot.org";
...
function normalizeSize3(size) {
  if (!size) return 3;
  return Math.min(Math.max(size, 3), 10);
}
function summarizeProteomeSearch(p) {
  return {
    id: p.id,
...
function summarizeProteome(p, max) {
  return {
    id: p.id,
    proteomeType: p.proteomeType,
    proteinCount: p.proteinCount,
    components: (p.components ?? []).slice(0, max).map((c) => ({
      name: c.name,
      proteinCount: c.proteinCount
    })),
    citations: (p.citations ?? []).slice(0, max).map((c) => ({
      id: c.id,
      title: c.title,
      journal: c.journal,
...
function summarizeProtein2(e2) {
  return {
    accession: e2.primaryAccession,
    uniProtkbId: e2.uniProtkbId,
    proteinName: e2.proteinDescription?.recommendedName?.fullName?.value ?? e2.proteinDescription?.submissionNames?.[0]?.fullName?.value,
    gene: e2.genes?.[0]?.geneName?.value,
    organism: e2.organism?.scientificName,
    taxonId: e2.organism?.taxonId,
    reviewed: e2.entryType === "UniProtKB reviewed (Swiss-Prot)",
    proteinExistence: e2.proteinExistence,
    length: e2.sequence?.length,
    sequenceVersion: e2.sequence?.version
  };
}
function summarizeGeneCentric(e2) {
  return {
    proteomeId: e2.proteomeId,
    canonicalAccession: e2.canonicalProtein?.id,
    proteinName: e2.canonicalProtein?.proteinName,
    gene: e2.canonicalProtein?.geneName,
    organism: e2.canonicalProtein?.organism?.scientificName,
    taxonId: e2.canonicalProtein?.organism?.taxonId,
    length: e2.canonicalProtein?.sequence?.length,
    entryType: e2.canonicalProtein?.entryType,
    proteinExistence: e2.canonicalProtein?.proteinExistence,
    relatedProteinCount: Array.isArray(e2.relatedProteins) ? e2.relatedProteins.length : 0
  };
}
var ProteinProteomesHandler = class {
  async run(args) {
    const size = normalizeSize3(args.size);
    let url;
    switch (args.action) {
      case "proteomes_search":
        if (!args.query) throw new Error("query required");
        url = `${BASE_URL4}/proteomes/search?query=${encodeURIComponent(
          args.query
        )}&size=${size}`;
        break;
      case "proteome_by_upid":
        if (!args.upid) throw new Error("upid required");
        url = `${BASE_URL4}/proteomes/${args.upid}`;
        break;
      case "proteins_by_proteome":
        if (!args.upid) throw new Error("upid required");
        url = `${BASE_URL4}/uniprotkb/search?query=proteome:${args.upid}&size=${size}`;
        break;
      case "proteins_by_gene":
        if (!args.query) throw new Error("gene name required");
        url = `${BASE_URL4}/uniprotkb/search?query=gene:${encodeURIComponent(
          args.query
        )}&size=${size}`;
        break;
      case "genecentric_search":
        if (!args.query) throw new Error("gene name required");
        url = `${BASE_URL4}/genecentric/search?query=${encodeURIComponent(
          args.query
        )}&size=${size}`;
        break;
      case "genecentric_by_accession":
        if (!args.accession) throw new Error("accession required");
        url = `${BASE_URL4}/genecentric/${args.accession}`;
        break;
      default:
    ...
    if (args.action === "proteome_by_upid") {
      ...
      result = Array.isArray(data.results) ? data.results.map(summarizeGeneCentric) : summarizeGeneCentric(data);
    } else {
      result = (data.results ?? []).map(summarizeProtein2);
    }
    return {
      structuredContent: {
        action: args.action,
...
// src/handlers/Protein/handlers/taxonomy.ts
var BASE_URL5 = "https://rest.uniprot.org";
var ProteinTaxonomyHandler = class {
  async run(args) {
    const {
      action,
      id,
    ...
    switch (action) {
      case "ancestor": {
        ...
        url = `${BASE_URL5}/taxonomy/ancestor/${encodeURIComponent(ids)}`;
        break;
      }
      case "by_id": {
        if (!id) {
        ...
        url = `${BASE_URL5}/taxonomy/id/${encodeURIComponent(id)}`;
        break;
      }
      case "children": {
        if (!id) {
        ...
        url = `${BASE_URL5}/taxonomy/id/${encodeURIComponent(id)}/children`;
        break;
      }
      case "children_node": {
        if (!id) {
        ...
        url = `${BASE_URL5}/taxonomy/id/${encodeURIComponent(
          id
        )}/children/node`;
        break;
      }
      case "node": {
        ...
        url = `${BASE_URL5}/taxonomy/id/${encodeURIComponent(id)}/node`;
        break;
      }
      case "parent": {
        if (!id) {
        ...
        url = `${BASE_URL5}/taxonomy/id/${encodeURIComponent(id)}/parent`;
        break;
      }
      case "parent_node": {
        if (!id) {
      ...
      case "siblings": {
        ...
        url = `${BASE_URL5}/taxonomy/id/${encodeURIComponent(id)}/siblings`;
        break;
      }
      case "siblings_node": {
        if (!id) {
      ...
      case "by_ids": {
        ...
        url = `${BASE_URL5}/taxonomy/ids/${encodeURIComponent(ids)}`;
        break;
      }
      case "by_ids_node": {
        if (!ids) {
        ...
        url = `${BASE_URL5}/taxonomy/ids/${encodeURIComponent(ids)}/node`;
        break;
      }
      case "lineage": {
        if (!id) {
        ...
        url = `${BASE_URL5}/taxonomy/lineage/${encodeURIComponent(id)}`;
        break;
      }
      case "by_name": {
        if (!name) {
        ...
        url = `${BASE_URL5}/taxonomy/name/${encodeURIComponent(name)}`;
        break;
      }
      case "by_name_node": {
        if (!name) {
        ...
        url = `${BASE_URL5}/taxonomy/name/${encodeURIComponent(
          name
        )}/node`;
        break;
      }
      case "path": {
        ...
        url = `${BASE_URL5}/taxonomy/path?${params.toString()}`;
        break;
      }
      case "path_nodes": {
        if (!from || !to) {
        ...
        url = `${BASE_URL5}/taxonomy/path/nodes?${params.toString()}`;
        break;
      }
      case "relationship": {
        if (!from || !to) {
        ...
        url = `${BASE_URL5}/taxonomy/relationship?${params.toString()}`;
        break;
      }
      default:
    ...
    if (!contentType.includes("application/json")) {
      const text = await res.text();
      throw new Error(
        "UniProt Taxonomy API returned non-JSON response: " + text.slice(0, 200)
      );
    }
    const data = await res.json();
...
// src/handlers/Protein/handlers/coordinates.ts
var BASE_URL6 = "https://rest.uniprot.org";
var ProteinCoordinatesHandler = class {
  async run(args) {
    const {
      action,
      taxonomy,
    ...
    switch (action) {
      case "search": {
        ...
        url = `${BASE_URL6}/coordinates?${params.toString()}`;
        break;
      }
      case "glocation_single": {
        ...
        url = `${BASE_URL6}/coordinates/glocation/${encodeURIComponent(
          taxonomy
      ...
      case "location_single": {
        ...
        url = `${BASE_URL6}/coordinates/location/${encodeURIComponent(
          accession
      ...
      case "by_accession": {
        ...
        url = `${BASE_URL6}/coordinates/${encodeURIComponent(accession)}`;
        break;
      }
      case "by_dbxref": {
        if (!dbtype || !dbid) {
        ...
        url = `${BASE_URL6}/coordinates/${encodeURIComponent(
          dbtype
    ...
    if (!contentType.includes("application/json")) {
      const text = await res.text();
      throw new Error(
        "UniProt Coordinates API returned non-JSON response: " + text.slice(0, 200)
      );
    }
    const data = await res.json();
    if (!data || Array.isArray(data) && data.length === 0) {
...
// src/handlers/Protein/handlers/uniparc.ts
var BASE_URL7 = "https://rest.uniprot.org";
function normalizeSize4(size) {
  if (!size) return 3;
  return Math.min(Math.max(size, 3), 10);
}
...
function trimUniParcEntry(entry) {
...
var ProteinUniParcHandler = class {
  async run(args) {
    const size = normalizeSize4(args.size);
    let data;
    if (args.action === "search") {
      ...
      const raw = await fetchJson3(
        `${BASE_URL7}/uniparc?${params.toString()}`
      );
      data = {
        total: raw.total,
      ...
      if (!args.accession) throw new Error("accession required");
      const raw = await fetchJson3(
        `${BASE_URL7}/uniparc/accession/${encodeURIComponent(
          args.accession
        )}?format=json`
      );
      ...
      if (!args.upid) throw new Error("upid required");
      const raw = await fetchJson3(
        `${BASE_URL7}/uniparc/proteome/${encodeURIComponent(
          args.upid
        )}?format=json`
      );
      data = trimResults(raw.results, size);
      ...
      const raw = await fetchJson3(
        `${BASE_URL7}/uniparc/upi/${encodeURIComponent(
          args.upi
        )}?format=json`
      );
      data = trimUniParcEntry(raw);
    ...
    return {
      structuredContent: {
        action: args.action,
        count: Array.isArray(data) ? data.length : Array.isArray(data?.results) ? data.results.length : 1,
        data
      },
      content: [
        {
...
// src/handlers/Protein/handlers/protein_accession_annotations.ts
var BASE_URL8 = "https://rest.uniprot.org";
var ProteinAccessionAnnotationsHandler = class {
  async run(args) {
    ...
    if (action === "search") {
      ...
      url = `${BASE_URL8}/${resource}?${params.toString()}`;
    } else if (action === "by_accession") {
      if (!accession) {
        throw new Error("accession is required for by_accession");
      }
      url = `${BASE_URL8}/${resource}/${encodeURIComponent(accession)}`;
    } else {
    ...
    if (!contentType.includes("application/json")) {
      const text = await res.text();
      throw new Error(
        `UniProt ${resource} API returned non-JSON response: ` + text.slice(0, 200)
      );
    }
    const data = await res.json();
    if (!data || Array.isArray(data) && data.length === 0) {
    ...
    const payload = {
      ...
      action,
      accession,
      count: Array.isArray(data.results) ? data.results.length : 1,
      data
    };
    return {
      content: [
