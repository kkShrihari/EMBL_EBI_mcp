...
export class EBISummaryMultiReferenceHandler {
    async run(args) {
        ...
        const url = `https://www.ebi.ac.uk/ebisearch/ws/rest/summary/api/multireference?` +
            params.toString();
        const res = await fetch(url);
        if (!res.ok) {
            throw new Error(`EBI summary multi-reference failed (${res.status})`);
        }
        ...
        if (!Array.isArray(data.links)) {
            throw new Error("Unexpected API response format: links not found");
        }
        const payload = {
...
//# sourceMappingURL=summary_multireference.js.map