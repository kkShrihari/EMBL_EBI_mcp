...
const allowedTargetsCache = new Map();
export class EBICrossReferenceTargetedHandler {
    async run(args) {
        ...
        if (domain === "uniprot" &&
            ...
            entryIds.includes("_")) {
            throw new Error("For UniProt XREF, use accession (e.g. P04637), not entry name (P53_HUMAN)");
        }
        // --------------------------------------------------
        ...
        if (!allowedTargetsCache.has(domain)) {
            ...
            if (!res?.structuredContent?.targetDomains ||
                !Array.isArray(res.structuredContent.targetDomains)) {
                throw new Error(`Failed to discover valid xref targets for domain '${domain}'`);
            }
            const targets = res.structuredContent.targetDomains.map((t) => t.domain);
        ...
        if (!allowedTargets.has(targetDomain)) {
            throw new Error(`Unsupported EBISearch cross-reference: ${domain} → ${targetDomain}`);
        }
        // --------------------------------------------------
        // Build request
        // --------------------------------------------------
        ...
        const url = `https://www.ebi.ac.uk/ebisearch/ws/rest/${encodeURIComponent(domain)}/entry/${encodeURIComponent(ids)}/xref/${encodeURIComponent(targetDomain)}?${params.toString()}`;
        const res = await fetch(url);
        if (!res.ok) {
            const text = await res.text();
            throw new Error(`EBI cross-reference search failed (${res.status}): ${text.slice(0, 200)}`);
        }
        ...
        const payload = {
            ...
            crossReferences: data.entries.map((e) => ({
                sourceId: String(e.id),
                targets: (e.references ?? []).map((x) => ({
                    id: String(x.id),
...
//# sourceMappingURL=xref_targeted.js.map