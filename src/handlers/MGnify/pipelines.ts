...
async function fetchSmart(url: string) {
  ...
  if (!res.ok) throw new Error(`MGnify pipelines failed (${res.status})`);
...
/* ---------------------------------
   HANDLER
---------------------------------- */
export class MGnifyPipelinesHandler {
  async run(args: {
    action:
      | "pipelines_list"
      | "pipelines_get"
    ...
    switch (args.action) {
      ...
      case "pipelines_get": {
        ...
        url = `${BASE_URL}/v1/pipelines/${args.release_version}${qs}`;
        break;
      }

      case "pipelines_analyses": {
        if (!args.release_version)
          throw new Error("release_version is required");
        const qs = buildQuery({
          format: args.format,
        ...
        url = `${BASE_URL}/v1/pipelines/${args.release_version}/analyses${qs}`;
        break;
      }

      case "pipelines_samples": {
        if (!args.release_version)
          throw new Error("release_version is required");
        const qs = buildQuery({
          format: args.format,
        ...
        url = `${BASE_URL}/v1/pipelines/${args.release_version}/samples${qs}`;
        break;
      }

      case "pipelines_tools": {
        if (!args.release_version)
          throw new Error("release_version is required");
        const qs = buildQuery({
          format: args.format,
        ...
        url = `${BASE_URL}/v1/pipelines/${args.release_version}/tools${qs}`;
        break;
      }

      default:
        throw new Error("Unknown pipelines action");
    }
    ...
    if (raw && (raw as any).__not_found) {
      const notFound = {
        action: args.action,
        release_version: args.release_version ?? null,
        data: [],
        note: `No data found for release_version=${args.release_version}`,
      };
    ...
    // Paginated endpoints
    if (
      args.action === "pipelines_list" ||
      args.action === "pipelines_analyses" ||
      args.action === "pipelines_samples" ||
      args.action === "pipelines_tools"
    ) {
    ...
    if (args.action === "pipelines_get") {
      const cleaned = clean(raw, 0);
      data = cleaned
        ? [
            {
              id: String(args.release_version),
              payload: cleaned,
            },
          ]
        : [];
