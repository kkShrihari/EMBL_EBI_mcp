...
async function fetchSmart(url: string) {
  ...
  if (!res.ok) throw new Error(`MGnify kegg-classes failed (${res.status})`);
...
/* ---------------------------------
   HANDLER
---------------------------------- */
export class MGnifyKeggClassesHandler {
  async run(args: {
    action: "kegg_classes_list" | "kegg_classes_get";
    id?: number;
    format?: "json" | "csv";
    page?: number;
    page_size?: number;
    ...
    switch (args.action) {
      case "kegg_classes_list": {
        ...
        url = `${BASE_URL}/v1/kegg-classes${qs}`;
        break;
      }

      case "kegg_classes_get": {
        if (args.id === undefined) throw new Error("id is required");
        const qs = buildQuery({ format: args.format });
        url = `${BASE_URL}/v1/kegg-classes/${args.id}${qs}`;
        break;
      }

      default:
        throw new Error("Unknown kegg-classes action");
    }
    ...
    if (args.action === "kegg_classes_list") {
      data = extractList(raw)
        .filter((x) => x.payload !== null)
        .slice(0, TOP_LIMIT);
    }

    if (args.action === "kegg_classes_get") {
