...
async function fetchSmart(url: string) {
  ...
  if (!res.ok) throw new Error(`MGnify pipeline-tools failed (${res.status})`);
...
/* ---------------------------------
   HANDLER
---------------------------------- */
export class MGnifyPipelineToolsHandler {
  async run(args: {
    action:
      | "pipeline_tools_list"
      | "pipeline_tools_get";
    ...
    switch (args.action) {
      case "pipeline_tools_list": {
        const qs = buildQuery({
          format: args.format,
          page: args.page,
          page_size: args.page_size,
        });
        url = `${BASE_URL}/v1/pipeline-tools${qs}`;
        break;
      }

      case "pipeline_tools_get": {
        if (!args.tool_name || !args.version)
          throw new Error("tool_name and version are required");

        const qs = buildQuery({ format: args.format });
        url = `${BASE_URL}/v1/pipeline-tools/${args.tool_name}/${args.version}${qs}`;
        break;
      }

      default:
        throw new Error("Unknown pipeline-tools action");
    }
    ...
    if (raw && (raw as any).__not_found) {
      const notFound = {
        ...
        data: [],
        note: `No data found for tool=${args.tool_name}, version=${args.version}`,
      };
    ...
    // Paginated list
    if (args.action === "pipeline_tools_list") {
    ...
    // Single tool
    if (args.action === "pipeline_tools_get") {
      const cleaned = clean(raw, 0);
      data = cleaned
        ? [
            {
              id: `${args.tool_name}@${args.version}`,
              payload: cleaned,
            },
          ]
        : [];
